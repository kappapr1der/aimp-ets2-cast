...
- Передаёт исполнителя и название текущего трека совместимым слушателям через ICY metadata.
- Клиенты без поддержки ICY продолжают получать обычный чистый MP3-поток.
- Ограниченная исходящая очередь сглаживает кратковременное заполнение
  локального сокета, не блокируя DSP и не разрывая соединение.
...
Версия 0.1.2 собрана и проверена в следующем окружении:
...
Релиз 0.1.2 предназначен только для x64 и не загрузится в 32-битный процесс AIMP. VLC для работы не нужен: он использовался только для независимой проверки потока.
...
1. Скачайте `aimp_ets2_cast-0.1.2.aimppack` со страницы GitHub Release.
...
Версия 0.1.2 намеренно публикует только endpoint `/stream`; запрос `/stream/` может вернуть `404 Not Found`.
...
Работа версии 0.1.0 в ETS2 подтверждена реальным пользователем, включая переключение треков. Версия 0.1.2 сохраняет тот же URL и MP3-формат. ICY metadata добавляется только по явному запросу клиента, поэтому прежние слушатели остаются совместимыми.

## Известные ограничения

- ETS2 1.61 воспроизводит поток, но не показывает ICY-название текущего трека. Совместимые внешние плееры получают метаданные; на звук в ETS2 это ограничение не влияет.
- Поток намеренно доступен только на этом компьютере.
- Релиз собран только для x64.
- В версии 0.1.2 адрес `/stream/` не равнозначен `/stream`.
...
Диагностический лог записывается сюда:

```text
%APPDATA%\AIMP\AIMP-ETS2-Cast\ets2cast.log
```

Если основной каталог недоступен для записи, плагин последовательно пробует `%LOCALAPPDATA%`, TEMP и каталог своей DLL. В лог попадают жизненный цикл, параметры PCM, стартовые тайминги, подключения слушателей и ошибки. PCM-данные и содержимое музыкальных файлов не записываются.
...
Отчёт о проверке версии 0.1.2 находится в [docs/VALIDATION_0.1.2.md](docs/VALIDATION_0.1.2.md). История версий — в [CHANGELOG.md](CHANGELOG.md).
